          ...
          (cond
            (i
             ...
             (let ((price (third (aref *mk-inventory* i)))
                   ...
                   (action "")
                   (jqi (make-jqi :target ".order .list")))
                
               (cond
                 (item
                  (incf (second item))
                  (incf (third item) price)
                  (setf (jqi-target jqi) (format nil ".order .list #~A" (first item))
                        action "replaceWith"))
                 (t
                  (setf item (list (format nil "~D~3,'0D" (get-universal-time) (incf *rdd-count*))
                        ...
                        action "prepend")))
               (add-jqi-m (make-jqi-m :action action
                                      :content (->html (order-item name item)))
                          jqi)
               (push (jqi->plist jqi) jqies)
               (incf *mk-order-ttp* price))

             ;; Change: Total-price
             (push (jqi->plist
                    (make-jqi :target ".order .total-price"
                              :methods (list
                                        (make-jqi-m
                                         :action "text"
                                         :content (format nil "¥~,2f" *mk-order-ttp*)))))
                   jqies)
             (<-json (list :|jQies| jqies)))
            (t
             ;; Error
             "")))))
...
(defstruct jqi-m
  ...
  (content 'null))
(defun jqi-m->plist (m)
  (list :|action| (jqi-m-action m)
        :|content| (jqi-m-content m)))
(defstruct jqi
  (target "")
  (methods nil))
(defun jqi->plist (jqi)
  (list :|target| (jqi-target jqi)
        :|methods| (mapcar #'jqi-m->plist (jqi-methods jqi))))

(defun add-jqi-m (m jqi)
  (push m (jqi-methods jqi)))
