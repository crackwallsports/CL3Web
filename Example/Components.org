   ...
   (ps*
    ...
    `(defun ws-start ()
       ...
       (setf (@ ws onmessage) (lambda (e)
                                ...
                                (case (@ json "MSG")
                                  ("movie" (ws-movie json))
                                  ("twitter-box" (ws-twitter-box json))
                                  (t ins))))
...
(defclass state ()
  ((text :initarg :text
         :initform ""
         :accessor state-text)
   (chars-remain :initarg :chars-remain
                 :initform 140
                 :reader state-chars-remain)
   (ins :initarg :ins
         :accessor state-ins)))
        ...
        (remove-if #'null
                   (list
                    ...
                    (make-ins ".box .chars-remain"
                              `("text" ,(format nil "~D" (- (state-chars-remain state)
                                                            (length value)))))))))
#+END_SRC
*** Data
#+BEGIN_SRC lisp :tangle yes
...
**** box
#+BEGIN_SRC lisp :tangle yes
(defun twitter-box-htm ()
  `(div (:class "box")
        (textarea (:class "text" :onkeyup "textChange(event)"))
        (div (:class "tools")
             (span (:class "spacing") "")
             (span (:class "chars-remain") "140")
             (button (:class "submit disabled") "Tweet"))
        (div (:class "test") "Test")))
...
*** Css
#+BEGIN_SRC lisp :tangle yes
(defun twitter-box-css ()
  '(".box" (:margin "auto 20px" :padding "10px 20px"
            :display "flex" :flex-flow "column"
            :box-shadow "0px 0px 1px 0px black")
    (".text" (:height "80px" :padding "10px"
                       :border-radius "5px"))
...
*** Js
#+BEGIN_SRC lisp :tangle yes
(defun twitter-box-js ()
  (ps* 
   `(defun ws-twitter-box (ins)
      (jq-ins ins))
