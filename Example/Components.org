...
(defmethod (setf state-val) (value (state state))
  (unless (equal value (slot-value state 'val))
    ...
    (call-observer state value)))

(defmethod call-observer ((state state) value)
  (maphash (lambda (k v)
             (declare (ignore k))
             (funcall v value))
   ...
   `(defun some-click (stat tag)
      (var msg (chain |json|
                 (stringify (create
                             ...
                             tag tag))))
      (chain ws (send msg)))
   `(defun change-list (obj stat tag)
      (var msg (chain |json|
                 (stringify (create
                             act "list"
                             stat stat
                             tag tag
                             val (chain ($ obj) (parent "li") (attr "id"))))))
      (chain ws (send msg)))))
#+END_SRC
  ...
  (case-str type
    ...
    ("list" (progn
               (if (and tag preprocess)
                   (progn 
                     (add-preprocess state tag preprocess)
                     (concat "changeList(this, '" (state-name state) "', '" tag "')"))
                   "")))))
...
#+BEGIN_SRC lisp
;; List State : name val
(defclass list-state (state)
  ((fitems :initform (make-hash-table :test #'equal))))

(defun make-list-state (name &optional val)
  (make-instance 'list-state
                 :name name
                 :val (loop for item in val
                         collect (make-state (format nil "~A" (random 100000))
                                             item))))
(defmethod add-fitem ((state state) tag f)
  (setf (gethash tag (slot-value state 'fitems)) f))
(defmethod rem-fitem ((state state) tag)
  (remhash tag (slot-value state 'fitems)))


(defmethod add-item ((state list-state) item)
  (let ((child (make-state (format nil "~A" (random 100000))
                           item)))
    (push child (slot-value state 'val))
    (maphash (lambda (tag f)
               (send-ins (list
                          (make-ins tag
                                    `("prepend" ,(->html (funcall f child)))))))
             (slot-value state 'fitems))))

(defmethod rem-item ((state list-state) (name string))
  (setf (slot-value state 'val) (remove-if (lambda (sname)
                                             (equal sname name))
                                           (state-val state) :key #'state-name))
  (send-ins (list (make-ins (concat "#" name) `("remove" :null)))))

(defun add-list-state (name val)
  (setf (gethash name *states*) (make-list-state name val)))


(add-state "text" "")
(add-list-state "todos"
                (list "看书"
                      "写字"
                      "锻炼"))
...
(defun vue-htm ()
  (let ((todos (get-state "todos"))
        (text (gethash "text" *states*)))
    `(div (:id "app-list")
          (input (:type "text" :id "input-text"
                        :onblur ,(data-> "input" text)))
          (button (:onclick ,(data-> "list" todos "add-item"
                                     (lambda (val)
                                       (unless (equal "" (state-val text))
                                         (add-item todos (state-val text))))))
                  "添加")
          (ol ()
              ,@(<-list-data "#app-list ol" todos
                             (lambda (todo)
                               (let ((id (state-name todo)))
                                 `(li (:id ,id)
                                      (span () ,(<-data (concat "#" id " span") todo))
                                      (span (:onclick ,(data-> "list" todos "rem-item"
                                                               (lambda (val)
                                                                 (rem-item todos val))))
                                            "X")))))))))
...
(defun <-list-data (tag state fitem &optional process)
  (let ((val (state-val state)))
    (if process
        (setf val (funcall process (state-val state))))
    (add-observer
     ...
     (lambda (val)
       (send-ins (list
                  (make-ins tag
                            `("html" ,(->html (loop for todo in (if process
                                                                    (funcall process val)
                                                                    val)
                                                 collect (funcall fitem todo)))))))))
    (and fitem
         (add-fitem state tag fitem)
         (loop for todo in val
            collect (funcall fitem todo)))))
  ...
  (let ((source (get-state "source"))
    ...
    `(div (:id "cryptography")
          ...
          (div (:class "actions")
               ...
               ;; Action
               (button (:onclick ,(data-> "click" result "action"
                                          (lambda (val)
                                            (declare (ignore val))
                                            (if (state-val action)
