...
(let ((message "Hello Vue!"))
  (defun vue-htm ()
    `(div (:id "app") ,message)))
;; OR
(defparameter *message* "Hello Vue!")
(defun vue-htm ()
  `(div (:id "app") ,*message*))

;; App-2
(let ((message (concat "页面加载于 " (human-date (get-universal-time)))))
  (defun vue-htm ()
    `(div (:id "app-2")
          ;; v-bind:title="message"  ?? change message -> title
          (span (:title ,message)))))
...
;; App-5
(defun vue-htm ()
  (with-slots (message) *state*
    `(div (:id "app-5")
          (p () ,message)
          ;; v-on:click="reverseMessage"
          (button (:onclick "reverseMessage()") "逆转消息"))))

(defun vue-js ()
  (ps*
   `(defun reverse-message ()
      (var msg (chain |json|
                 (stringify (create
                             act "reverse-message"
                             val ""))))
...
;; State
(defclass state ()
  ((message :initform "Hello Vue.js!"
            :accessor state-message)
   (ins :initarg :ins
        :accessor state-ins)))

(defmethod (setf state-message) :before (value (state state))
  (setf (state-ins state)
        (nconc (state-ins state)
               (remove-if #'null
                          (list
                           (make-ins "#app-5 .p"
                                     `("text" ,value)))))))
...
(defun reverse-message ()
  (setf (state-message *state*) (reverse (state-message *state*))))
...
(defun vue-htm ()
  (with-slots (message) *state*
    `(div (:id "app-6")
          (p () ,message)
          ;; v-model="message"
          (input (:onchange "input-change()")))))
;; 双向绑定 需要类似 App-5 的过程
#+END_SRC
**** 组件化应用构建
#+BEGIN_SRC lisp
;; App-7
(let ((grocery-list ((:id 0 :text "蔬菜")
                     (:id 1 :text "奶酪")
                     (:id 2 :text "随便其他什么人吃的东西"))))
  (defun vue-htm ()
    `(div (:id "app-7")
          (ol ()
              ,@(loop for item in grocery-list
                   collect (todo-item-htm item))))))
...
(defmethod preprocess ((state state) tag)
  (funcall (gethash tag (slot-value state 'prep))))
...
(defun <-data (tag state &optional process)
  (let ((val (state-val state)))
    ...
    (add-observer state
                  tag
                  (lambda (val)
                    (send-ins (list (make-ins tag `("text" ,val))))))
    val))
...
;; View
(setf (gethash "word" *states*) (make-state "word" "hello World!"))
